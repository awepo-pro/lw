...
> Roadmap files: [`PLAN-v1.md`](PLAN-v1.md) (v1.0 — next) · [`PLAN-v1.1.md`](PLAN-v1.1.md) (v1.1 — after that).
...
| D6 | **Plain word search**, no SQLite, no FTS5 | An in-memory inverted index is instant at vault scale and removes a cgo-free-but-still-heavy dependency. FTS5 → `PLAN-v1.md` |
| D7 | **One open changeset at a time**, single-threaded ingest | Matches git's index; keeps review comprehensible. Parallel ingest → `PLAN-v1.md` |
...
Graph view is v1.0 (`PLAN-v1.md`). Vim-first keymap, superfile-style `hotkeys.toml` +
...
search are in `PLAN-v1.md` for when it isn't.
...
| Word search proves insufficient | Contained: `wiki.search` is one interface. FTS5 and embeddings are drop-ins (`PLAN-v1.md`) |
