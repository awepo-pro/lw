...
numbers below (`/PLAN.md §N`) cite the design of record; nothing here invents
a rule that isn't traceable to `/PLAN.md` or `01-backbone.md`.

## 1. Directory layout (`/PLAN.md §6`, backbone §1)
...
`wiki/summaries` is not enumerated in `/PLAN.md §6`'s tree but is one of the
five `PageType` values in backbone §2.1 (`TypeSummary`), whose `Dir()` is
`"wiki/summaries"`; it is listed here for completeness.

## 2. Wiki page frontmatter (`/PLAN.md §6`; field order and enums from
backbone §2.1-§2.2)
...
| `tags` | list of strings | emitted unless empty (then `[]`) | every tag must be present in `SCHEMA.md`'s taxonomy (`/PLAN.md §6`, backbone §2.2 `Validate`) |
...
## 3. Raw source frontmatter (`/PLAN.md §6`; fields from backbone §2.7 `RawSource`)
...
| `sha256` | string | hex sha256 of the body; drives dedupe and drift detection (`/PLAN.md §6`) |
...
## 4. Mechanical conventions (`/PLAN.md §6`)
...
- Tags outside `SCHEMA.md`'s taxonomy are **rejected at `stage.*` proposal
  time**, not merely discovered later at lint time (`/PLAN.md §6`, backbone
  §5.5 `ValidateOp`).
