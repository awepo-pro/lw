...
// TestLogProfile asserts the pane re-resolves its cursor tint when the
...
func TestLogProfile(t *testing.T) {
	t.Run("ansi256_cursor_row_uses_grey", func(t *testing.T) {
		p := publicPane(t, true)
		q, _ := p.Update(tea.ColorProfileMsg{Profile: colorprofile.ANSI256})

		styled, _ := uitest.PaneScreen(q, 120, 38)
